...
export default function DopeTechEcommerce() {
  ...
  const handleCategoryClick = useCallback((categoryId: string) => {
    ...
    setTimeout(() => {
      ...
      if (productsSection) {
        ...
        const categoriesHeight = 120 // Approximate categories section height including padding
        const elementTop = productsSection.getBoundingClientRect().top + window.pageYOffset
        const offsetPosition = elementTop - headerHeight - categoriesHeight - 40 // Keep categories visible with more space
  ...
  return (
    <div className="text-white min-h-screen transition-colors duration-100" style={{ background: 'linear-gradient(135deg, #000000 0%, #1a1a1a 30%, #1a1a0a 50%, #1a1a1a 70%, #000000 100%)' }}>
      <CursorTracker />
      ...
      {/* Welcome Section */}
      <section className="pt-16 sm:pt-20 md:pt-24 lg:pt-28 pb-8 sm:pb-12 md:pb-16 relative" style={{ background: 'linear-gradient(135deg, #000000 0%, #1a1a0a 50%, #000000 100%)' }}>
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          ...
          <div className="text-center mb-6 sm:mb-8 md:mb-12">
            ...
            <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-300 mb-4 sm:mb-6 md:mb-8">
              Premium Tech Gear from <span className="text-[#F7DD0F]">DopeTech</span> Nepal
            </p>
            
            {/* Autoplay GIF */}
            <div className="w-full max-w-6xl mx-auto mb-4 sm:mb-6 md:mb-8">
              <img
          ...
          {/* Category Filter */}
          <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2 md:gap-3 mb-4 sm:mb-6 md:mb-8 px-2">
            {categories.map((category) => (
              <div key={category.id} className="relative">
                <button
                  onClick={() => handleCategoryClick(category.id)}
                  className={`flex items-center space-x-1.5 px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 md:py-3 rounded-full transition-all duration-100 cursor-hover text-xs sm:text-sm md:text-base ${
              ...
              <div 
                ...
                className="grid gap-3 sm:gap-4 md:gap-6 lg:gap-8 grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filteredProducts.map((product) => (
              <div key={product.id} className="group">
                <div className="dopetech-card p-3 sm:p-4 md:p-6 h-full flex flex-col">
                  {/* Product Image */}
                  <div className="relative mb-2 sm:mb-3 md:mb-4">
                    <img
                      src={product.image}
                      alt={product.name}
                      className="w-full h-24 sm:h-32 md:h-48 object-cover rounded-lg transition-transform duration-100 group-hover:scale-105"
                      loading="lazy"
                    />
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="mt-auto">
                      ...
                      <button
                        ...
                        className={`w-full py-3 px-4 rounded-lg font-medium transition-all duration-100 ${
                          product.inStock
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-50 flex">
          ...
          {/* Cart Panel */}
          <div className="relative ml-auto w-full max-w-sm sm:max-w-md bg-white dark:bg-[#1a1a1a] shadow-2xl">
